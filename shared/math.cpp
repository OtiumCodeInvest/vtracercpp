// Otium Code Shared Lib Math v1.10.0
// - 2026/09/02 (1.10.0) - VMax/VMin for VI2
...
void VI2::SetRound(V2& v){
	x=(int)roundf(v.x);y=(int)round(v.y);
}
V2 VI2::GetV2()const{
...
// Backfacing or nearly parallel?
	if((fabsf(a)<=FEPSILON)) return false;
...
void HermiteCurve::SetPoints(const V3* pvPositions,int lNumberPositions) {
	...
	for(a=0;a!=m_lNumberPoints;a++) {
		m_pPoint[a].m_position=pvPositions[a];
		m_pPoint[a].m_vVelocity=V3(0,0,0);
	}

	for(a=1;a<m_lNumberPoints-1;a++) {
		V3 v0=m_pPoint[a-1].m_position;
		V3 v1=m_pPoint[a].m_position;
		V3 v2=m_pPoint[a+1].m_position;
		V3 vPrev=v1-v0;
		V3 vNext=v2-v1;
	...
	for(a=0;a!=m_lNumberPoints-1;a++) {
		m_pPoint[a].m_fDistanceToNextPoint=VLength(m_pPoint[a+1].m_position-m_pPoint[a].m_position);
		m_fTotalLength+=m_pPoint[a].m_fDistanceToNextPoint;
	}
}
V3 HermiteCurve::GetPositionAtLength(float fLength)const {
	if(fLength<0)
		return m_pPoint[0].m_position;
	if(fLength>m_fTotalLength)
		return m_pPoint[m_lNumberPoints-1].m_position;
	int a;
	for(a=0;a!=m_lNumberPoints-1;a++) {
	...
	//V3 v0=Lerp(m_pPoint[a].m_position,m_pPoint[a+1].m_position,t);
	V3 v0=VHermite(m_pPoint[a].m_position,m_pPoint[a].m_vVelocity,m_pPoint[a+1].m_position,m_pPoint[a+1].m_vVelocity,0,m_pPoint[a].m_fDistanceToNextPoint,fLength);
	return v0;
}
...
M33 MatrixRotate(const M33& m,const V3& axis, float ang ) {
	...
	float fsinf = (float)sinf( ang );
	float fX, fY, fZ;
	M33 MRotate;
	//
	// Normalize the vector
	//
	V3 axisn = VNormalize( axis );
	fX = axisn.x;
	fY = axisn.y;
	fZ = axisn.z;
 
	MRotate.m[0][0] = (fX * fX) * (1.0f - fCos) + fCos;
	MRotate.m[0][1] = (fX * fY) * (1.0f - fCos) - (fZ * fsinf);
	MRotate.m[0][2] = (fX * fZ) * (1.0f - fCos) + (fY * fsinf);
 
	MRotate.m[1][0] = (fY * fX) * (1.0f - fCos) + (fZ * fsinf);
	MRotate.m[1][1] = (fY * fY) * (1.0f - fCos) + fCos ;
	MRotate.m[1][2] = (fY * fZ) * (1.0f - fCos) - (fX * fsinf);
 
	MRotate.m[2][0] = (fZ * fX) * (1.0f - fCos) - (fY * fsinf);
	MRotate.m[2][1] = (fZ * fY) * (1.0f - fCos) + (fX * fsinf);
	MRotate.m[2][2] = (fZ * fZ) * (1.0f - fCos) + fCos;
   
	//MRotate.m[0][3] = MRotate.m[1][3] = MRotate.m[2][3] = 0.0f;
	//MRotate.m[3][0] = MRotate.m[3][1] = MRotate.m[3][2] = 0.0f;
	//MRotate.m[3][3] = 1.0f;
	return Multiply(MRotate,m);
}
...
// Checks if an oriented bounding box is (partial or completely) inside a set of clip planes
// Clip planes are assumed to have the normal pointing out

enum ECheckInsideFlag 
{
	CHECK_INSIDE_FULLY_OUTSIDE =	0x0,
	CHECK_INSIDE_PARTIALLY_INSIDE =	0x1,
	CHECK_INSIDE_FULLY_INSIDE =		0x2,
};

inline V4 VMergeXY(const V4& v1,const V4& v2) { 
	V4 res;
	res.x=v1.x;
	res.y=v2.x;
	res.z=v1.y;
...
inline V4 VNormalizePlane(const V4& v) {
	V4 vRcpLength=1.0f/VLength(v);
	return v*vRcpLength;
}

V4 VXXZX(const V4& v){return V4(v.x,v.x,v.z,v.x);}
V4 VXXYZ(const V4& v){return V4(v.x,v.x,v.y,v.z);}
V4 VZXXZ(const V4& v){return V4(v.z,v.x,v.x,v.z);}
V4 VYXXZ(const V4& v){return V4(v.y,v.x,v.x,v.z);}
V4 VXZXZ(const V4& v){return V4(v.x,v.z,v.x,v.z);}
V4 VXYXZ(const V4& v){return V4(v.x,v.y,v.x,v.z);}

inline void MCreateClipPlanesNormalized(V4* pPlanes,const M44& mProjection) {
	M44 mTranspose=MTranspose(mProjection);
	static const V4 v={0,1,-1,0};
	pPlanes[0]=VNormalizePlane(TransformVec4(mTranspose,VXXZX(v)));//Front
	pPlanes[1]=VNormalizePlane(TransformVec4(mTranspose,VXXYZ(v)));//Back
	pPlanes[2]=VNormalizePlane(TransformVec4(mTranspose,VZXXZ(v)));//Left
	pPlanes[3]=VNormalizePlane(TransformVec4(mTranspose,VYXXZ(v)));//vRight
	pPlanes[4]=VNormalizePlane(TransformVec4(mTranspose,VXZXZ(v)));//vBottom
	pPlanes[5]=VNormalizePlane(TransformVec4(mTranspose,VXYXZ(v)));//vTop
}
inline void MCreateClipPlanes(V4* pPlanes,const M44& mProjection)
{
	M44 mTranspose=MTranspose(mProjection);
	static const V4 v={0,1,-1,0};
	pPlanes[0]=TransformVec4(mTranspose,VXXZX(v));//Front
	pPlanes[1]=TransformVec4(mTranspose,VXXYZ(v));//Back
	pPlanes[2]=TransformVec4(mTranspose,VZXXZ(v));//Left
	pPlanes[3]=TransformVec4(mTranspose,VYXXZ(v));//vRight
	pPlanes[4]=TransformVec4(mTranspose,VXZXZ(v));//vBottom
	pPlanes[5]=TransformVec4(mTranspose,VXYXZ(v));//vTop
}


// Checks if an oriented bounding box is (partial or completely) inside a set of clip planes
// Clip planes are assumed to have the normal pointing out
// vRadius must have radius in x,y,z and w
// vCenter must have w=1

inline bool CheckSphereInsidePlanes(const V4* pvPlanes,size_t nNumPlanes,const V4& vCenter,const V4& vRadius) {
	uint32 i;
	for(i=0;i<nNumPlanes;i++) {
		V4 vDist=VDot(vCenter,pvPlanes[i]);
		if(vRadius-vDist<V4(0,0,0,0)) {
			break;
		}
	}
	return (i==nNumPlanes);
...
inline ECheckInsideFlag CheckOBBInsidePlanes(const V4* pvPlanes,uint32 nNumPlanes,const M44& mObject,const V3& vCenterLocal,const V4& vHalfSizeLocal) {
	//V4 vCenter=VTransform43(mObject,vCenterLocal);
	V4 center=V4(TransformCoord(vCenterLocal,mObject),1);
	V4 vZero=V4(0,0,0,0);//VLoadZero();
	bool bInsideAll=true;
	uint32 i;
	for(i=0;i<nNumPlanes;i++) {
		V4 vDist=VDot(center,pvPlanes[i]);
		V4 vX=VDot(mObject.XAxis(),pvPlanes[i]);
		V4 vY=VDot(mObject.YAxis(),pvPlanes[i]);
		V4 vZ=VDot(mObject.ZAxis(),pvPlanes[i]);
		V4 vXZ=VMergeXY(vX,vZ);
		V4 vYW=VMergeXY(vY,vZero);
		V4 vXYZ=VMergeXY(vXZ,vYW);
		V4 vVal=VDot(VAbs(vXYZ),vHalfSizeLocal);
		if(vVal-vDist<vZero) {
			break;
		}
		// inside all
		bInsideAll &= (vVal+vDist<vZero);
	}

	if(i == nNumPlanes) {
		// made it all the way,no break
		return bInsideAll ? CHECK_INSIDE_FULLY_INSIDE : CHECK_INSIDE_PARTIALLY_INSIDE;
	} else {
		// we hit a break,entirely invisible
		return CHECK_INSIDE_FULLY_OUTSIDE;	
	}
}
...
//M44 m=MLoad(M33::Identity(),pos);

//MCreateClipPlanesNormalized(planes,MLoadIdentity());

//bool f=CheckSphereInsidePlanes(planes,6,V4(0,3.1f,0,1),V4(1,1,1,1));

//ECheckInsideFlag f=CheckOBBInsidePlanes(planes,6,m,V3(0,0,0),V4(1,1,1,1));
//uprintf("flag %d\n",f);
